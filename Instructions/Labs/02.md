...
In this exercise, you will understand & learn how to create, run and analyze the results of the queries based on specific hunts thus unveiling the possible attack points and threat mitigations.
...
5. Notice that the **Result count** post the query runs within the metrics bar of the **Microsoft sentinel | Hunting** page. These results of the query runs appear for specific query logs having been identified.
...
2. Once installed, select **Log4J Vulnerability Detection** solution and perform the following:
   ...
   >**Note**: The **Add to existing hunt (Preview)** option can be chosen in order to add queries from the solution into an existing hunt.
...
3. Staying on the current **Hunt** page has a list of all the selected queries which were added to the hunt in the previous step, click on **Run all queries**.
